...
response configuration}; ``morphological body'' is shorthand.  Its atemporal
selection is not a stored trajectory or observer clock.  The transport and
recovery results below begin only after this typed body packet is supplied.
...
Record transport may inherit a legacy descriptor (L) only across Paper I's
exact fibre square:
\begin{equation}
 ...
 \label{eq:record-legacy-migration}
\end{equation}
Equality of the two fibre relations gives exact descriptor equivalence; on a
regular smooth branch the local test is
$\ker D\Xi_{OS}^{\rm cont}\subseteq\ker DL$. Equal rank is insufficient:
$\Xi(x,y)=x$ and $L(x,y)=x+y$ are rank one but do not factor. A hard record
edge must additionally preserve the stable cells of $Q_{OS,\delta}$.
Therefore successful transport of selected records cannot derive the
physical quantizer, calibration, $q_R(R)$, or Nature occupation.
...
The word ``descriptor'' in this definition denotes the frozen operational
protocol, including its terminal records and tolerances. It is not the smooth
shared parent descriptor $\Xi_{OS}^{\rm cont}$. The two are connected only
by $D_{OS}^{\rm op}=Q_{OS,\delta}(\Xi_{OS}^{\rm cont})$, with the quantizer
and calibration supplied as physical inputs rather than inferred from
complete-edge or recovery success.
...
The same restriction applies to the migration theorem above: it transports
already supplied terminal structure but does not construct the physical
resolution cells or any galactic radial transfer function.
...
The current finite laboratory is a reproducible implementation, not a new Nature-selection theorem. The new modular prior uses real ratios $\exp(x_b-x_a)$, whose oriented cycle products telescope to one. It is therefore not a construction of nontrivial Bargmann phase, non-Abelian holonomy or physical curvature. The complex action's global phase circle likewise does not by itself supply a phase-sensitive detector.
